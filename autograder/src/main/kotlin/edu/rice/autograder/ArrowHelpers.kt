...
internal fun <T> Try<T>.onFailure(consumer: (Throwable) -> Unit): Try<T> = fold({ consumer(it); this }) { this }

/** Runs the given lambda on the value contained inside a [Try]'s success. Returns the original Try. */
internal fun <T> Try<T>.onSuccess(consumer: (T) -> Unit): Try<T> = fold({ this }) { consumer(it); this }

/** Converts a [Try] to a [List] of one or zero elements */
internal fun <T> Try<T>.asList() = fold({ emptyList<T>() }, { listOf(it) })

/** Converts a [Try] to a [Sequence] of one or zero elements */
internal fun <T> Try<T>.asSequence() = fold({ emptySequence<T>() }, { sequenceOf(it) })

/** Extracts a [Try] success value or (re-throws) an exception. */
internal fun <T> Try<T>.getOrFail(): T = fold({ throw it }, { it })

/** Extracts an [Option] some()'s value or throws an exception on none(). */
internal fun <T> Option<T>.getOrFail(): T = fold({ throw IllegalStateException("getOrFail() on an Option.none") }) { it }
...
internal fun <T, K : Any> Iterable<T>?.associateNotNullBy(keySelector: (T) -> K?): Map<K, T> =
    if (this == null)
        emptyMap()
    else
        flatMap {
